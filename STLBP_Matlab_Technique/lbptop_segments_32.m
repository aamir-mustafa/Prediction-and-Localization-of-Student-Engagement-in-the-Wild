...
person=dir('../vggfaces/faces')
for l=3:size(person,1)
    
person_name=person(l).name;

face_path=strcat('../vggfaces/faces/',person_name);



 % please replace "..." by your images path
all_faces = dir(strcat(face_path,'/*.bmp')); % directory of images, ".jpg" can be changed, for example, ".bmp" if you use
count_jj=1;
lbp_top_person=zeros(177,59);
 for jj=1:10:590
  

           a=all_faces(jj:jj+19,1);
...
for i = 1 : length(a)
    ...
    Imgdat = imread(strcat(face_path,'/',ImgName));
...
lbptop_feats=zeros(3*59,1);
for k=1:3
    lbptop_feats(1+59*(k-1):59*(k),1)=Histogram(k,:);
...
lbp_top_person(:,count_jj)=lbptop_feats;
        l
        count_jj
        
        count_jj=count_jj+1;
 end
 save_name=strcat(person_name,'_lbptop.mat');
 save(save_name,'lbp_top_person');
